...
/* ========================================================
   1️⃣  Build (or rebuild) the welcome headline
   ======================================================== */
function buildWelcomeHeadline() {
    ...
    data.words.forEach((w, i) => {
        if (w === '<br>') {
            // <br> is a *token* – insert a real <br> element
    ...
    // 3️⃣ stick the headline into the section
    section.appendChild(h1);
}

  /* --------------------------------------------------------------------
     3.  Build the <h1> with <span.word> children
     -------------------------------------------------------------------- */
  buildWelcomeHeadline();   // <‑ this will create the headline exactly as you want
...
/* section.innerHTML = '';        // clear old content
  ...
  data.words.forEach((w, i) => {
    if (w === "<br>") {
      ...
      const span = document.createElement('span');
      span.textContent =  w;  
	// prepend a space except for the first word. before it was simply "=(i > 0 ? ' ' : '') + w;" 
      span.className   = 'word';
      h1.appendChild(span);
    }
  });
...
*/
  ...
  restoreBtn.addEventListener('click', () => {
    const original = data.words;
    words.forEach((w, i) => {
      w.style.transitionDelay = `${i * 100}ms`;
      w.textContent = original[i];
      w.classList.remove('show');
      setTimeout(() => w.classList.add('show'), i * 100 + 50);
    });
  });
...
/* --------------------------------------------------------------------
   3.  Load intro‑text files (used by the .intro containers in the
      section files – not called automatically, but kept for future use)
   -------------------------------------------------------------------- */
  ...
  fetch(src)
    ...
    .then(text => {
      ...
      introItem.className = 'item intro-item';   // uses the CSS we just added
     	 	// **Here we turn the plain text into HTML**
      		// 1. replace real line breaks (\n) with <br>
      		// 2. set innerHTML so that <br> tags in the file are rendered
      introItem.innerHTML = text.replace(/\n/g, '<br>');
       // introItem.textContent = text;
